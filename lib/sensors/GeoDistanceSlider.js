			...
			var execQuery = function execQuery() {
				if (_this6.props.onValueChange) {
					_this6.props.onValueChange({
						input: _this6.state.currentValue,
						distance: _this6.state.currentDistance,
						location: _this6.locString,
						unit: _this6.props.unit
					});
				}
				helper.selectedSensor.setSortInfo(sortObj);
				helper.URLParams.update(_this6.props.componentId, _this6.setURLValue(), _this6.props.URLParams);
		...
		if (input) {
			...
			this.autocompleteService.getPlacePredictions(options, function (res) {
				res.forEach(function (place) {
					_this8.result.options.push({
						label: place.description,
						value: place.description
					});
				});
				if (_this8.state.userLocation.length && _this8.result.options[0].label !== "Use my current location") {
					_this8.result.options.unshift({
						label: "Use my current location",
						value: _this8.state.userLocation
					});
				}
				_this8.callback(null, _this8.result);
			});
		} else {
